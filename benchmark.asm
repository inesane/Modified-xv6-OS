...
printf(int fd, const char *fmt, ...)
{
 ...
 664:	eb 1a                	jmp    680 <printf+0x1a0>
          s = "(null)";
 666:	bb 6b 08 00 00       	mov    $0x86b,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 6b5:	a1 24 0b 00 00       	mov    0xb24,%eax
{
 ...
 6d6:	73 10                	jae    6e8 <free+0x38>
{
 ...
 727:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 72a:	a3 24 0b 00 00       	mov    %eax,0xb24
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 74d:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 750:	8b 3d 24 0b 00 00    	mov    0xb24,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 799:	8b 3d 24 0b 00 00    	mov    0xb24,%edi
 ...
 7c2:	e8 e9 fe ff ff       	call   6b0 <free>
  return freep;
 7c7:	a1 24 0b 00 00       	mov    0xb24,%eax
 ...
 7f4:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 7f7:	a3 24 0b 00 00       	mov    %eax,0xb24
}
 ...
 817:	0b 00 00 
    base.s.size = 0;
 81a:	bf 28 0b 00 00       	mov    $0xb28,%edi
    base.s.ptr = freep = prevp = &base;
 81f:	c7 05 28 0b 00 00 28 	movl   $0xb28,0xb28
 826:	0b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 829:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 82b:	c7 05 2c 0b 00 00 00 	movl   $0x0,0xb2c
 832:	00 00 00 
    if(p->s.size >= nunits){
